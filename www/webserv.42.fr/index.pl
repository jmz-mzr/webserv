...
if ($ENV{'REQUEST_METHOD'} eq 'POST') {
  if (defined $cgi->param('clearbtn')) {
    $session->delete();
    $session->flush();
    $cookie = CGI::Cookie->new(-name => 'CGISESSID', -value => $session->id(), -expires => '-1d');
	print "Location: http://$ENV{'HTTP_HOST'}$ENV{'REQUEST_URI'}#welcome\r\n\r\n";
  }
...
END
print qq(
  <h3 class="w3-center w3-text-teal">WELCOME DEAR @{[uc($session->param('username'))]}</h3>
